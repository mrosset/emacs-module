...
simple, all it does is take a C string literal called ~CMOD_VERSION~ and
passes it to Emacs ~(message)~ function. It's the same as if we called
~(message "0.1")~. And the end result will display 0.1 in the message
window and buffer.
