...
#+OPTIONS: toc:t num:3 H:3 ^:nil pri:nil html-style:nil
...
#+HTML_HEAD: <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.100.2/js/materialize.min.js"></script>

#+TITLE: Writing Emacs modules with Go

#+DOWNLOADED: http://orgmode.org/img/org-mode-unicorn-logo.png @ 2017-09-05 10:16:22
[[file:img/org-mode-unicorn-logo_2017-09-05_10-16-22.png]]

This page is generated using ~(org-html-export-to-html)~ if you prefer using
...
Next is the actual C function that we plan to call from Emacs. Our function is
pretty simple all it does is take a C string literal called ~CMOD_VERSION~ and
passes it to Emacs ~(message)~ function. It's pretty much the same as if we
called ~(message "0.1")~. And the end result will display 0.1 in the message
window and buffer.

#+BEGIN_SRC C :main no :tangle src/cmodule.c
  static emacs_value
  Fcmodule_version (emacs_env * env, int nargs,
