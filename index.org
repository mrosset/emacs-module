...
it will work for the function we want to create.
#+BEGIN_SRC go :main no :tangle src/main.go
  static void
...
#+BEGIN_SRC shell
export CGO_CPPFLAGS="-I $PWD/emacs-25.2/src"
go build -buildmode=c-shared -o lib/gmodule.so src/main.go
#+END_SRC

#+RESULTS:

You can also set the CFLAGS in your go files using cgo directives. For more
information on cgo directives and cgo see the [[https://golang.org/cmd/cgo/][CGO]] web page.
